...
# This scripts runs GCC as well as cc on each test-case provided and compares the results.
#
...
# - in the TEST step, we actually run GCC and cc on each test-case
#
#
...
description = "Compile multiple programs with both GCC and cc, run them, and compare the results.",
epilog      = ""
)
...
argparser.add_argument('-w','--wrapper',metavar='PATH',
                       help='Invoke your compiler through the shell script at PATH. (default: `cc-wrapper.sh`)')
...
orig_cwd=os.getcwd()
if "cc-test-output" in orig_cwd:
    print('error: cannot run from within the output directory')
    exit(1)
    
if os.path.isdir('cc-test-output'):
    # cleanup previous output directory
    command('rm -rf cc-test-output')
os.mkdir('cc-test-output')
...
if args.wrapper:
    ...
    wrapper=os.path.dirname(os.path.realpath(__file__))+"/cc-wrapper.sh"
...
######################################################################################
## PREPARE step: copy all test-cases under cc-test-output
...
for inputfilename in inputfilenames:
    ...
    if 'cc-test-output' in os.path.realpath(inputfilename):
    ...
    ## ../somedir/subdir/file.c becomes ./cc-test-output/somedir-subdir-file/input.c
    subdir='cc-test-output/'+inputfilename.strip("./")[:-2].replace('/','-')
    os.mkdir(subdir)
...
for jobname in jobs:
    ...
    ## cc compiler
    ccstatus=command(wrapper+" asm-cc.s input.c", "cc-compile.txt")
    
    if gccstatus != 0 and ccstatus != 0:
        ## cc correctly rejects invalid program -> test-case ok
        print("TEST OK")
        continue
    elif gccstatus != 0 and ccstatus == 0:
        ## cc wrongly accepts invalid program -> error
        print("TEST FAIL (your compiler accepts an invalid program)")
        continue
    elif gccstatus == 0 and ccstatus != 0:
        ## cc wrongly rejects valid program -> error
        print("TEST FAIL (your compiler rejects a valid program)")
        if args.verbose:
            dumpfile("cc-compile.txt")
        continue
    else:
        ## cc accepts to compile valid program -> let's link it
        ldstatus=command("gcc -o exe-cc asm-cc.s", "cc-link.txt")
        if ldstatus:
            print("TEST FAIL (your compiler produces incorrect assembly)")
            if args.verbose:
                dumpfile("cc-link.txt")
            continue

    ## both compilers  did produce an  executable, so now we  run both
    ## these executables and compare the results.
        
    command("./exe-cc","cc-execute.txt")
    if open("gcc-execute.txt").read() != open("cc-execute.txt").read() :
        print("TEST FAIL (different results at execution)")
        if args.verbose:
            print("GCC:")
            dumpfile("gcc-execute.txt")
            print("you:")
            dumpfile("cc-execute.txt")
        continue
