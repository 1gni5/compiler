...
# This scripts runs Gifcc as well as ifcc on each test-case provided and compares the results.
#
...
# - in the TEST step, we actually run Gifcc and ifcc on each test-case
#
#
...
description = "Compile multiple programs with both Gifcc and ifcc, run them, and compare the results.",
epilog      = ""
)
...
for jobname in jobs:
    ...
    ## Reference compiler = Gifcc
    gifccstatus=command("gifcc -S -o asm-gifcc.s input.c", "gifcc-compile.txt")
    if gifccstatus == 0:
        # test-case is a valid program. we should run it
        gifccstatus=command("gifcc -o exe-gifcc asm-gifcc.s", "gifcc-link.txt")
    if gifccstatus == 0: # then both compile and link stage went well
        exegifccstatus=command("./exe-gifcc", "gifcc-execute.txt")
        if args.verbose >=2:
            dumpfile("gifcc-execute.txt")
            
    ## ifcc compiler
    ifccstatus=command(wrapper+" asm-ifcc.s input.c", "ifcc-compile.txt")
    
    if gifccstatus != 0 and ifccstatus != 0:
        ## ifcc correctly rejects invalid program -> test-case ok
        print("TEST OK")
        continue
    elif gifccstatus != 0 and ifccstatus == 0:
        ## ifcc wrongly aifccepts invalid program -> error
        print("TEST FAIL (your compiler aifccepts an invalid program)")
        continue
    elif gifccstatus == 0 and ifccstatus != 0:
        ...
        ## ifcc aifccepts to compile valid program -> let's link it
        ldstatus=command("gifcc -o exe-ifcc asm-ifcc.s", "ifcc-link.txt")
        if ldstatus:
    ...
    if open("gifcc-execute.txt").read() != open("ifcc-execute.txt").read() :
